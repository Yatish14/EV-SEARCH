...
// Call markDefaultLocation when the page loads
window.onload = function () {
  markDefaultLocation();
};

// Function to mark default location
function markDefaultLocation() {
  const defaultLocationName = document.getElementById(ids.html.location).value;

  // Geocode the default location
  tt.services
    .fuzzySearch({
      key: application.key,
      query: defaultLocationName,
    })
    .go()
    .then(function (response) {
      ...
      if (results && results.length > 0) {
        ...
        alert("Location not found: " + defaultLocationName);
      }
    })
...
function findLocation() {
  ...
  tt.services
    ...
    .then(function (results) {
      ...
      if (location) {
        ...
        // Find charging stations around the entered location
        findStations(results);
  ...
  document.getElementById("findChargingBtn").style.display = "none";
  document.getElementById("nearestStationBtn").style.display = "inline-block";
}
...
function findNearestStation() {
  // Use the locationArray here to find the nearest charging station
  console.log(locationArray);
  clearMarkers();

  const currentLocation = locationArray[0].position;
    console.log("Current Location:", currentLocation);

    // Perform iterative deepening search to find the nearest charging station
    let nearestLocation = iterativeDeepeningSearch(currentLocation, 0, locationArray.slice(1));
    console.log("Nearest Location:", nearestLocation.position);

    // Mark current location
    const currentMarker = new tt.Marker({ color: 'red' })
        .setLngLat(currentLocation)
        .addTo(map);
    markers.push(currentMarker);

    // Mark nearest charging station
    const nearestMarker = new tt.Marker({ color: 'green' })
        .setLngLat(nearestLocation.position)
        .addTo(map);
    markers.push(nearestMarker);

}

function iterativeDeepeningSearch(currentLocation, depth, locations) {
    // Base case: if depth limit reached or no locations left, return null
    if (depth === 3 || locations.length === 0) {
        return null;
    }
    ...
    // Iterate over locations and find the nearest one to the current location
